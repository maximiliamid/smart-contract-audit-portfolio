...
contract PuppetV3Challenge is Test {
    ...
    function test_puppetV3() public checkSolvedByPlayer {
        // Uniswap V3 oracle is TWAP-based but the lending pool uses a very
        // short observation window. Dump all 110 DVT into the V3 pool to
        // crash spot price, skip past the TWAP window (~110s), then borrow.

        // Approve + swap all DVT for WETH via V3 pool's swap interface
        token.approve(
        ...
        // Advance time to bias the TWAP toward the manipulated spot
        skip(110);

        // Wrap player's ETH into WETH, then borrow the whole pool
